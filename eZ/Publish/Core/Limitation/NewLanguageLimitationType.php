...
use eZ\Publish\API\Repository\Values\User\Limitation\NewLanguageLimitation as APINewLanguageLimitation;
use eZ\Publish\API\Repository\Values\User\Limitation as APILimitationValue;
use eZ\Publish\SPI\Limitation\Type as SPILimitationTypeInterface;
use eZ\Publish\Core\FieldType\ValidationError;
use eZ\Publish\API\Repository\Values\Content\Query\Criterion;
use eZ\Publish\SPI\Persistence\Content\UpdateStruct as SPIUpdateStruct;

/**
 * LanguageLimitation is a Content limitation.
 */
class NewLanguageLimitationType extends AbstractPersistenceLimitationType implements SPILimitationTypeInterface
{
    /**
    ...
    {
        if (!$limitationValue instanceof APINewLanguageLimitation) {
            throw new InvalidArgumentType('$limitationValue', 'APINewLanguageLimitation', $limitationValue);
    ...
    {
        return new APINewLanguageLimitation(array('limitationValues' => $limitationValues));
    }
    ...
    {
        if (!$value instanceof APINewLanguageLimitation) {
        ...
        foreach ($targets as $target) {
            if ($target instanceof ContentUpdateStruct || $target instanceof SPIUpdateStruct) {
                if (\in_array($target->initialLanguageCode, $value->limitationValues, true)) {
                    dump($target->initialLanguageCode);
                    dump($value->limitationValues);
                    return true;
                }
